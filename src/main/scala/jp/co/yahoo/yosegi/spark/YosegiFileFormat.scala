...
class YosegiFileFormat extends FileFormat with DataSourceRegister with Serializable{
  ...
  override def buildReaderWithPartitionValues(
    ...
    ( file: PartitionedFile ) => {
      ...
      if( expandOption.nonEmpty ){
        throw new RuntimeException( "Spark 2.2.0 does not support Expand and Flatten." );
      }
      if( flattenOption.nonEmpty ){
        throw new RuntimeException( "Spark 2.2.0 does not support Expand and Flatten." );
      }
      ...
      var reader:IColumnarBatchReader = new SparkColumnarBatchReader( partSchema.asInstanceOf[StructType] , file.partitionValues , readSchema.asInstanceOf[StructType] , fs.open( path ) , fs.getFileStatus( path ).getLen() , file.start , file.length , yosegiConfig , node )
